...
// BMP 파일 헤더 구조체
#pragma pack(push, 1) //pragma  데이터 따닥따닥 붙여주는거 ?  fileheader 한묶음 , info한묶음 빈공간없이 만듬
typedef struct {
...
int main() {
	const char* inputFile = "brainct_001.bmp"; // brainct_001.bmp
	const char* outputFile = "output_grayscale.bmp"; //output_grayscale.bmp
	...
	// 입력 파일 열기
	if (fopen_s(&inFile, inputFile, "rb") != 0 || inFile == NULL) {  //0일떄 파일이 열어짐 파일있다 . 1= 파일없음 안열어짐
	...
	fread(&fileHeader, sizeof(BMPFileHeader), 1, inFile); // 사이즈 바이트 수만큼 읽는다 fread <- 이거 는  
	...
	// BMPFileHeader 내용 출력
	printf("--- 파일 헤더 (BMPFileHeader) ---\n");
	printf("파일 형식 (type)     : %X (16진수)\n", fileHeader.type); // 'BM'을 의미하는 4D42가	보여야 합니다.
	printf("파일 크기 (size)     : %u 바이트\n", fileHeader.size);
	printf("데이터 오프셋 (offset) : %u\n", fileHeader.offset);

	// BMPInfoHeader 내용 출력
	printf("\n--- 정보 헤더 (BMPInfoHeader) ---\n");
	printf("정보 헤더 크기 (size)   : %u\n", infoHeader.size);
	printf("이미지 너비 (width)   : %d\n", infoHeader.width);
	printf("이미지 높이 (height)  : %d\n", infoHeader.height);
	printf("픽셀당 비트 수 (bitCount) : %hu\n", infoHeader.bitCount);
	printf("압축 방식 (compression) : %u\n", infoHeader.compression);
	printf("이미지 데이터 크기 (sizeImage) : %u 바이트\n", infoHeader.sizeImage);
